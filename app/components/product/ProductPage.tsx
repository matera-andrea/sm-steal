...
function ProductSkeleton() {
  return (
    <div className="max-w-7xl mx-auto px-6 py-20 grid grid-cols-1 lg:grid-cols-12 gap-12 animate-pulse">
      <div className="lg:col-span-7 aspect-square bg-gray-100 rounded-[3rem]" />
      <div className="lg:col-span-5 space-y-8 mt-20">
        <div className="h-4 bg-gray-100 w-1/4 rounded" />
        <div className="h-24 bg-gray-100 w-3/4 rounded-3xl" />
        <div className="h-12 bg-gray-100 w-1/2 rounded-xl" />
        <div className="grid grid-cols-3 gap-3">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="h-24 bg-gray-100 rounded-2xl" />
          ))}
        </div>
        <div className="h-16 bg-gray-100 w-full rounded-full" />
      </div>
    </div>
  );
}
...
function ProductNotFound() {
  return (
    <div className="min-h-[70vh] flex flex-col items-center justify-center space-y-6 text-center px-4">
      <h2 className="text-5xl md:text-7xl font-black uppercase italic tracking-tighter text-gray-200">
        Sold Out.
      </h2>
      <p className="text-gray-500 font-medium">
        This product is currently unavailable or doesn&apos;t exist.
      </p>
      <Link
        href="/shop"
        className="bg-black text-white px-8 py-3 rounded-full text-xs font-black uppercase tracking-widest hover:bg-amber-400 hover:text-black transition-colors"
      >
        Back to Shop
      </Link>
    </div>
